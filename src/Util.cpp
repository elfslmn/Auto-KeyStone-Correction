    ...
    namespace util {
            ...
            T thresh, int iterations, int num_points)
        {
           //std::cout <<"points size: "<<points.size()<<std::endl;
            ...
            for (int iteration = 0; iteration < iterations; ++iteration) {
                ...
                const cv::Vec<T, 3> & a = points[idx[0]], &b = points[idx[1]], &c = points[idx[2]];
                //std::cout <<iteration <<": "<< a <<"\t"<<b<<"\t"<<c<<std::endl;
            ...
            cv::parallel_for_(cv::Range(0, R*C), [&](const cv::Range & range) {
                for (int r = range.start; r < range.end; ++r) {
                    ...
                    output_mat.ptr<Vec3f>(i)[j] =
                        util::normalAtPoint(xyz_map,Point2i(j, i) * (resolution / step), normal_dist);
                }
            });
        ...
        double surfaceArea(const cv::Size & frame_size,
            ...
            for (unsigned i = 0; i < cluster_size; ++i) {
               //printf("(%d,%d)",points_ij[i].x, points_ij[i].y);
                    ...
                    if (points_ij[nx].x < points_ij[j].x) {
                       //printf("2");
                        ...
                        nx = std::lower_bound(it1, it2, Point2i(points_ij[j].x, points_ij[nx].y),
                            PointComparer<Point2i>(0, true))- points_ij.begin();
                    }
        ...
        ***/
            ...
            int inv1, int inv2, float inv2_thresh, cv::Mat * color)
        {
            ...
            color->at<uchar>(seed) = 1; //is being visited (1), has already been visited (0) or is not yet visited (255)
            ...
            while (stkSize > 0) {
                ...
                while (visPtr[pt.x] > 0) {
                    ...
                    for (uint i = 0; i <= nNext; ++i) {
                        ...
                        if (util::norm(*xyz - xyz_map.at<Vec3f>(adjPt)) <(i < 2 ? thresh : max_distance2)) {
                            stk[stkSize++] = adjPt;
                    ...
                    if (sw) {
                        // go right
                        pt.x += inv1;
                        if (pt.x >= C || visPtr[pt.x] == 0 || util::norm(*xyz - xyzPtr[pt.x]) >= thresh)
                        {
                            sw = false;
        ...
        Vec3f myRansac(const std::vector<Vec3f>& points, const std::vector<cv::Point2i>& indices,
            float thresh, int iterations, int num_points)
        {
           //std::cout <<"points size: "<<points.size()<<std::endl;
            Vec3f best; int bestInliers = 0;
            ...
            for (int iteration = 0; iteration < iterations; ++iteration) {
                ...
                const Vec3f & a = points[idx[0]], &b = points[idx[1]], &c = points[idx[2]];
                //std::cout <<iteration <<": "<< a <<"\t"<<b<<"\t"<<c<<std::endl;

                // compute equation of plane through these points
                Vec3f normal = cv::normalize((b - a).cross(c - a));
                if (normal[2] > 0) normal = -normal;

                float k = (normal[0] * a[0] + normal[1] * a[1]) / normal[2] + a[2];
                Vec3f eqn(-normal[0] / normal[2], -normal[1] / normal[2], k);

                // compute number of inliers
                int inliers = 0;
                for (int i = 0; i < num_points; ++i) {
                    float norm = util::pointPlaneNorm(points[i], eqn);
                    if (norm < thresh) ++inliers;
                }
            ...
            Mat map;
            map.create(Size(224,171), CV_8UC1);
            map = Scalar::all (0);

            for (int i = 0; i < num_points; ++i) {
                float norm = util::pointPlaneNorm(points[i], best);
                if (norm < thresh){
                   map.at<uint8_t>(indices[i]) = (uint8_t)255;
                }
            }
            imshow("Inliers",map);
