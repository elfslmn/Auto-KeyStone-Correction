...
namespace ark {
...
#ifdef DEBUG
        cv::imshow("[Hand Defects Debug]", visual);
#endif
    ...
    int Hand::touchingPlane(const FramePlane & plane, std::vector<int> & output, bool extrapolate) const
    {
        output.clear();

        for (int i = 0; i < getNumFingers(); i++)
        {
            bool touching = plane.touching(fingersXYZ[i], fingersIJ[i], params->touchThreshold, !extrapolate);
            if (touching) {
                output.push_back(i);
            }
        }
    ...
    int Hand::touchingPlanes(const std::vector<std::shared_ptr<FramePlane>>& planes,
        std::vector<std::pair<int, std::vector<int> >>& output, bool extrapolate) const
    {
        output.clear();
        ...
        for (int i = 0; i < getNumFingers(); i++)
        {
            ...
            for (int j = 0; j < planes.size(); ++j) {
                ...
                bool touching = plane.touching(fingersXYZ[i], fingersIJ[i],
                    params->touchThreshold, !extrapolate);
